...
def read_data(file_path):
    ...
    with open(file_path, 'r') as file:
        while True:
            ...
            labels.append(encode_wire_color(label))
...
def main():
    ...
    X_train, y_train = read_data("C:\\Users\\nasus\\Desktop\\Intro to AI\\Project3\\data\\BonusData\\TaskTwoTraining2000.txt") 

    # Read testing data, 
    X_test, y_test = read_data("C:\\Users\\nasus\\Desktop\\Intro to AI\\Project3\\data\\BonusData\\TaskTwoTesting.txt")

    # Convert diagrams to feature space for training data
    X_train_feature_space = np.array([get_overrlap_feature_space(diagram) for diagram in X_train])
    X_test_feature_space = np.array([get_overrlap_feature_space(diagram) for diagram in X_test])


    model = LogisticRegression(multi_class='multinomial', max_iter=MAX_ITERATIONS)

    # Flatten the one-hot encoded labels to 1-dimensional   
    y_train_flat = np.argmax(y_train, axis=1)
    y_test_flat = np.argmax(y_test, axis=1)

    model.fit(X_train_feature_space, y_train_flat)
    ...
    classification_report_str = classification_report(y_test_flat, y_pred)
